...
**[short instruction]**
```
cd ~ && mkdir -p jenkins/jenkins_home/ jenkins/logs/nginx/ jenkins/certs/ jenkins/registry REPO/external/
cd ~/REPO/external && git clone https://github.com/gardli/jenkins-nginx-docker.git
...
Docker version check between host and docker image has been disabled as it brings extra complexity to the setup. Concidering jenkins containerisation itself and attached jenkins_home volume - it's more handy to create new image with docker version update on host.

As we will not check docker version, jenkins container can be built now using `docker-compose` [[3](https://github.com/docker/compose/releases)].
